...
class ReviewQueueReportTests(RetinopathyTestCaseMixin):
    ...
    def _add_dicom(self, eye_exam_register, file_type: str) -> None:
        SessionFile.objects.create(
            eye_exam_register=eye_exam_register,
            file_type=file_type,
            original_filename=f"{file_type}.dcm",
            stored_filename=f"{eye_exam_register.pk}_{file_type}.dcm",
            file_size=1024,
    ...
    def test_review_status(self) -> None:
        self.assertEqual(
            _review_status(reviewed=False, uploaded=False),
            "Awaiting results",
        )
        self.assertEqual(
            _review_status(reviewed=False, uploaded=True),
            "Ready for review",
        )
    ...
    def test_rows_cover_three_states_ordered_by_subject(self) -> None:
        ...
        reviewed_session = self.create_eye_exam_register(rs_reviewed)
        self._add_dicom(reviewed_session, "right_dicom")
        self._add_dicom(reviewed_session, "left_dicom")
        DmRetinopathyScreening.objects.create(eye_exam_register=reviewed_session)
    ...
    def test_summary_counts(self) -> None:
        ...
        DmRetinopathyScreening.objects.create(eye_exam_register=reviewed)
